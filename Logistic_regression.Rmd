  ---
title: "RMarkdown Python Setp-up Linear Regression..."
author: "Madhurya Jagadeesh"
date: "2024-01-31"
...
- `Smarket` data, which is part of the `ISLR2` library

- Consists of percentage returns for the S\&P 500 stock index
...
```{r chunk0003, error=TRUE, echo=TRUE}
cor(Smarket)
cor(Smarket[, -9])
```
...
```{r chunk0009, echo=TRUE}
table(glm.pred, Direction)
(507 + 145) / 1250 # true classification ( diagnonal) # 52% is the output which is not good prediction outcome as even the predictors r not so significant
...
- However, this result is misleading because we trained and tested the  model on the same set of $1,250$ observations. In other words,
...
- Notice that the syntax for the `lda()` function is identical to that of `lm()`, and to that of `glm()` except for the absence of the `family` option. We fit the model using only the observations before 2005.
